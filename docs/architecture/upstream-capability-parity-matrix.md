...
| --- | --- | --- | --- |
| YAML、剪贴板、URL/订阅导入与导出 | `pubspec.yaml`、`android/`、`core/` | 配置导入、校验、备份恢复、错误回退 | Partial：真机识别文件配置；确认二维码、文件、URL 添加入口及 SAF 导出入口；未完成导出，且固定 Android UI 未见独立剪贴板入口 |
| 订阅更新、重命名、复制、删除和持久化 | `core/`、配置服务 | 配置仓库和版本化变更 | Partial：真机确认编辑、预览、覆写和删除入口；当前为文件配置，未验证订阅更新、重命名或复制行为 |
| 规则/全局/直连模式 | `core/Clash.Meta`、配置路径 | 模式选择与状态持久化 | Partial：真机确认三种模式；全局模式经强停重启保持，结束时恢复规则模式；直连仅确认入口 |
| 代理组展开、节点切换和当前节点 | `core/`、Android bridge | 代理组/节点页面与运行状态 | Partial：真机显示多个代理组和节点卡；切换节点产生持久状态变更并可重启，当前节点未能通过无敏感语义独立复核 |
| 节点协议、地区、标签筛选 | `core/` | 筛选、排序和可用性标记 | Pending |
...
| 请求、规则、内核日志与崩溃诊断 | `core/`、service bridge | 脱敏诊断导出 | Partial：公开 HTTPS 流量后请求页显示 10 个可见记录；当前设置下工具页未显示日志入口，未验证规则日志或崩溃诊断 |
| 内核版本、更新和回滚 | `core/Clash.Meta`、更新路径 | 组件管理、校验和、回滚 | Pending |
| 启动连接、自动更新、快捷方式/小组件 | `android/`、平台集成 | 后台入口和设备能力开关 | Partial：真机确认自动运行和自动检查更新开关；固定 Android 包未声明静态快捷方式或 AppWidget，相关能力为 Unavailable |
