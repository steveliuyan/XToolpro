...
| Android VPN 启停与竞争 VPN 处理 | `android/service`、`android/core` | `VpnService` 生命周期和权限流程 | Partial：真机多次完成启动、停止和恢复；规则模式下应用退到后台后，系统 VPN 与 `tun0` 归属仍存在，公开 HTTPS 返回 `204` 后仍保持连接，回到应用可正常停止并清理。未验证竞争 VPN、首次授权拒绝或撤销 |
