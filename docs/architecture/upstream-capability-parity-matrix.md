...
| Android 13+ 前台通知权限门控 | `android/app/.../AndroidManifest.xml`、`AppPlugin.kt`、`ServiceState.kt` | VPN 启动前权限、拒绝与 unavailable 状态 | Partial：固定 manifest 声明 `POST_NOTIFICATIONS`，且 API 33 设备的 `POST_NOTIFICATION` app-op 当前为 `allow`；源码在启动路径请求该权限，但 permission callback 不检查 `permissions` 或 `grantResults`，而是将进程内 skip 标志置位并无条件继续启动；没有附着 Activity 时同样直接继续。故固定包当前获准状态不证明拒绝、撤销、冷启动或回调异常能安全映射为用户可见 unavailable/cancel 状态。XToolpro 必须以真实授权结果和前台通知可见性为门控，拒绝时保持不启动并给出可恢复状态；不得复用该 unconditional-continue 分支。真机未改动授权、未启动 VPN，也未读取通知内容 |
