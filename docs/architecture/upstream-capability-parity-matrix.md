...
| 请求/连接列表详情字段与筛选 | `lib/views/connection/connections.dart`、`lib/views/connection/item.dart`、`lib/models/common.dart`、`lib/core/interface.dart` | 请求/连接详情、搜索筛选和隐私边界 | Partial：固定源码的 `TrackerInfo`/`Metadata` 模型包含 UID、进程与进程路径、网络类型、源/目的 IP 与端口、host、规则及 payload、代理链、源/目的 GeoIP、ASN、DNS mode、special proxy/rules 和 remote destination；连接页每秒调用 `coreController.getConnections()` 拉取完整列表，详情页直接展示上述字段。列表搜索仅覆盖 network、host、destination IP、process 和 proxy chains，未覆盖源/目的端口、规则 payload、GeoIP/ASN、DNS 或 remote destination；读取异常会将原始 `$error` 写入 `commonPrint`。本轮未读取设备连接正文或网络标识，仅复核固定源码；该能力入口与字段覆盖可确认，但尚无字段级脱敏、最小化返回或稳定错误映射证据，保持 `Partial` |
