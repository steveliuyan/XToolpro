...
| 按应用代理/绕过 | `core/`、Android package bridge | 应用选择与路由策略 | Partial：真机进入访问控制页，页面说明选中应用将被排除在 VPN 之外；“允许应用绕过 VPN”初始为开启，临时关闭后强停重启仍为关闭，关闭态可建立并停止 `VpnService`/`tun0`，恢复开启后再次强停重启仍为开启；固定源码把该值传入 `VpnOptions.allowBypass` 并在建 VPN 时调用 `VpnService.Builder.allowBypass()`。2026-09-07 当前固定包在目标设备未获取到可选应用列表，ADB UI tree 仅见页面说明、无应用条目或错误/重试控件；包已声明且被授予 `QUERY_ALL_PACKAGES`，根因未确定。Android 13 `dumpsys` 未暴露 `allowBypass` 字段，故未选择应用或验证应用主动绕过后的实际流量路径 |
