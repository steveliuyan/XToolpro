...
| Android bridge 日志与错误字段边界 | `android/common/.../GlobalState.kt`、`android/service/.../VpnService.kt`、`android/app/.../ServiceState.kt`、`ServiceController.kt`、`ServiceBroadcastReceiver.kt` | 结构化、脱敏的诊断事件和稳定错误码 | Partial：固定 `GlobalState.log(text)` 直接调用 `Log.d("FlClash", text)`；`VpnService` 将 `route.address`/`prefixLength` 写入日志，core setup 的 message/exception 以及 service bind/start/stop/disconnect/unbind、broadcast action 的异常字符串也未经字段脱敏直接拼接日志。静态源码不能证明任一运行时消息必含订阅 URL、凭据、Cookie、节点名或请求内容，本轮未读取 logcat、应用日志、配置、节点、订阅 URL、凭据、Cookie 或请求；XToolpro 必须使用稳定错误码、默认关闭敏感诊断并在日志/导出边界统一脱敏，完成契约测试前保持 `Partial` |
