...
| TUN、系统代理、局域网共享 | `android/service/src/main/java/com/follow/clash/service/VpnService.kt`、`core/`、Android bridge | 运行模式和局域网开关 | Partial：`tun0` 与真实 HTTPS 流量已验证；真机“系统代理”为开启，运行态 Android VPN `LinkProperties` 报告 loopback `7890` HTTP proxy，停止后对应代理消失；设备在运行态显式通过 `127.0.0.1:7890` HTTP 代理并禁用 `NO_PROXY` 访问公开 HTTPS，返回 `204`，并以同一端口的 SOCKS5 hostname 模式再次返回 `204`，证明该 loopback mixed port 可实际转发两种 TCP 代理协议。“局域网代理”为关闭时 mixed port 的 4 条 TCP 监听均仅绑定 loopback且无 wildcard；临时开启后强停重启仍为开启，运行态 mixed port 有 8 条监听且 wildcard `:7890` 计数为 1，公开 HTTPS 返回 `200`，停止并恢复关闭后再次强停重启仍为关闭。该 proof 只验证本机 loopback TCP 转发和监听边界，不从其他局域网设备发起连接，故不外推为 UDP 或跨设备共享访问通过 |
