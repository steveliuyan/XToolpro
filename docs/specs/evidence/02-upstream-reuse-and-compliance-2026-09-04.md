...
### FlClash proxy-provider cache/ETag 文件权限与并发生命周期静态审计（2026-09-10）

- 固定归档文件 SHA-256：`core/Clash.Meta/component/profile/cachefile/cache.go`=`F78DB1582F01FAAFF1BB6260398A2A747809ABFF30C67EE03AC0F477ADE3227B`；`etag.go`=`477D1BBBE97EF51DDE00ABEBF7B82293006F60EC34EF4BE54C790914A4670D6A`。
- 固定 cache DB 以宽松 `0666` 模式打开，未见 owner/group/ACL 收紧、目录权限核验、跨进程单实例锁、打开/关闭幂等或停止后的句柄回收回执；ETag/subscription 元数据与缓存内容也未按 profile/provider 细分访问边界。
- 因此并发更新、多实例打开、进程崩溃或停止期间，固定路径不能证明锁等待/写入顺序、旧句柄关闭、敏感缓存残留或恢复重建已经收敛；权限失败与锁冲突也没有稳定终态。
- 结论：XToolpro adapter 必须在私有目录以最小权限打开受保护存储，建立单实例/锁生命周期、关闭清理和可回收句柄确认，并将权限拒绝、锁冲突、取消、崩溃和版本错配映射为脱敏结果；完成权限、并发、多实例、崩溃恢复和清理契约测试前保持 `Partial`，Proxy 台账保持 `Investigating`。
- 本轮未打开或读写 cache DB、未发起 HTTP 请求、未解析真实 provider、未启动 core、未使用 ADB，也未读取设备日志、配置、节点、URL、地址、路由、DNS、流量、凭据、Cookie、数据库内容或文件内容。

#### 本检查点远端备份状态（2026-09-10，FlClash proxy-provider cache/ETag 文件权限与并发生命周期静态审计）

- focused commit 待创建；本检查点只涉及 capability parity matrix 与本 evidence 文件，其他工作树改动和临时产物不纳入；按要求不得自动 push，须先获得用户明确确认。

### FlClash proxy-provider healthcheck 结果生命周期与重启恢复静态审计（2026-09-10）

- 固定归档关键路径：`core/Clash.Meta/adapter/provider/healthcheck.go`、`core/Clash.Meta/adapter/provider/provider.go`；沿用已记录的固定提交和文件哈希，仅补充结果生命周期边界。
- 固定 healthcheck 结果附着于运行时 proxy/healthcheck 对象；当前证据未见持久化快照、样本时间、过期标记、provider/config generation 或重启恢复。`Close()` 只取消内部 context，不等待 in-flight 检查，也不输出最后一次结果。
- provider 替换、规则或测试 URL 变化后，调用方不能仅凭 alive/delay 区分新鲜、过期、未执行和上一代结果；固定路径没有 unknown/stale 的稳定状态或可取消终态。
- 结论：XToolpro adapter 必须绑定 provider/config generation，持久化最小化结果和 freshness 上限，重启后显式标记 unknown/stale，并提供脱敏、可取消的终态；完成时间推进、重启恢复、provider 替换和取消契约测试前保持 `Partial`，Proxy 台账保持 `Investigating`。
- 本轮未运行 healthcheck、未发起测速或网络请求、未启动 core、未使用 ADB，也未读取设备日志、配置、节点、URL、地址、路由、DNS、流量、凭据、Cookie、数据库或文件。

#### 本检查点远端备份状态（2026-09-10，FlClash proxy-provider healthcheck 结果生命周期与重启恢复静态审计）
...
### FlClash proxy-provider healthcheck 状态码/延迟归一化静态审计（2026-09-10）

- 固定归档关键路径：`core/Clash.Meta/adapter/provider/healthcheck.go`、`core/Clash.Meta/adapter/provider/provider.go`；沿用已记录的固定提交和文件哈希，仅补充响应结果边界。
- 固定 healthcheck 以 HTTP 响应与耗时更新 proxy alive/delay，但未见统一状态码 allowlist、延迟负值/溢出/极值校验、异常响应体大小限制或跨协议结果归一化；expectedStatus、网络错误和 timeout 的组合没有逐项稳定分类。
- 结论：XToolpro adapter 必须限制响应和延迟范围，定义状态码与异常值映射，禁止原始响应进入日志，并以脱敏 `Success`/`Unavailable`/`Cancelled`/`VersionMismatch` 回执区分非预期状态、测量无效和取消；完成状态码、极值、错误响应体和取消契约测试前保持 `Partial`，Proxy 台账保持 `Investigating`。
- 本轮未运行 healthcheck、未发起测速或网络请求、未启动 core、未使用 ADB，也未读取设备日志、配置、节点、URL、地址、路由、DNS、流量、凭据、Cookie、数据库或文件。

#### 本检查点远端备份状态（2026-09-10，FlClash proxy-provider healthcheck 状态码/延迟归一化静态审计）

- focused commit 待创建；本检查点只涉及 capability parity matrix 与本 evidence 文件，其他工作树改动和临时产物不纳入；按要求不得自动 push，须先获得用户明确确认。

### FlClash proxy-provider 更新与 healthcheck 代际竞态静态审计（2026-09-10）

- 固定归档关键路径：`core/Clash.Meta/adapter/provider/provider.go`、`core/Clash.Meta/adapter/provider/healthcheck.go`；沿用已记录的固定提交和文件哈希，仅补充 stale-result 边界。
- provider `setProxies` 替换切片并递增 version 后异步触发 healthcheck；固定 worker 结果未见携带 provider/config generation、对旧快照完成结果做 stale 检查，或在替换期间阻止旧 proxy 状态回写。更新、检查、关闭之间未见统一顺序和冲突回执。
- 结论：XToolpro adapter 必须让每轮检查绑定不可变 snapshot/generation，在提交结果前验证代次，旧结果丢弃并记录脱敏计数，替换失败时保留旧快照；完成更新期间并发检查、stale write、取消和回滚契约测试前保持 `Partial`，Proxy 台账保持 `Investigating`。
- 本轮未运行 healthcheck、未触发 provider 更新、未发起测速或网络请求、未启动 core、未使用 ADB，也未读取设备日志、配置、节点、URL、地址、路由、DNS、流量、凭据、Cookie、数据库或文件。

#### 本检查点远端备份状态（2026-09-10，FlClash proxy-provider 更新与 healthcheck 代际竞态静态审计）
...
### FlClash External-controller provider PUT 回执与 healthcheck 完成时序静态审计（2026-09-10）

- 固定归档关键路径：`core/Clash.Meta/hub/route/provider.go`、`core/Clash.Meta/adapter/provider/provider.go`、`core/Clash.Meta/adapter/provider/healthcheck.go`；沿用已记录的固定提交和文件哈希，仅补充更新回执边界。
- 固定 `PUT /providers/{name}` 在 `provider.Update()` 返回后立即以空正文 204 响应；后续 healthcheck/状态收敛没有任务 ID、generation、完成时间或逐节点结果可回读。204 只表示调用链返回，不能证明新 proxy 快照已健康、旧结果已失效或失败后已回滚。
- 结论：XToolpro adapter 必须把 provider PUT 建模为可取消任务，返回版本化 receipt，分离解析成功、快照发布、healthcheck 完成和失败回滚，并以脱敏终态报告；契约测试完成前保持 `Partial`，Proxy 台账保持 `Investigating`。
- 本轮未发送管理请求、未触发 provider 更新或 healthcheck、未发起网络请求、未启动 core、未使用 ADB，也未读取设备日志、配置、节点、URL、地址、路由、DNS、流量、凭据、Cookie、数据库或文件。

#### 本检查点远端备份状态（2026-09-10，FlClash External-controller provider PUT 回执与 healthcheck 完成时序静态审计）
...
### FlClash subscription-userinfo 缓存失效、重命名与清理静态审计（2026-09-10）

- 固定归档关键路径：`core/Clash.Meta/adapter/provider/provider.go`、`core/Clash.Meta/adapter/provider/subscription_info.go`、`core/Clash.Meta/component/profile/cachefile/subscriptioninfo.go`；沿用已记录的固定提交和文件哈希，本轮只补充 provider 生命周期变更边界。
- 固定实现以 provider name 作为 `subscriptioninfo` bucket 的缓存键，初始化按当前 name 恢复原始 `subscription-userinfo` header；已记录的路径没有显示重命名时的 key 迁移或别名映射，也没有显示删除 provider、移除来源、URL 变更或配置重载时的 invalidate/purge 钩子。
- 缓存没有 TTL、撤销标记、来源指纹、provider/config generation、孤儿扫描或清理数量/失败回执；因此旧 name 的条目可能在重命名后继续被恢复，来源变更时也没有可核验的“旧值不可用”终态。固定路径未定义缓存命中与当前来源不一致时应保留、标记 stale/unknown 还是拒绝的稳定语义。
- 结论：XToolpro adapter 必须使用稳定 provider ID 与来源代次绑定缓存，重命名通过原子迁移并保留可回滚记录；删除、来源/URL 变更和配置移除必须执行可核验 purge，过期、撤销、孤儿和清理失败必须映射为脱敏 `Unavailable`/`Cancelled`/`VersionMismatch` 或明确的 stale/unknown 摘要。完成重命名、删除、URL 变更、TTL/撤销和清理回执契约测试前，新增 parity 行保持 `Partial`，Proxy 台账保持 `Investigating`。
- 本轮仅基于固定归档记录进行静态审计，未触发 provider 重命名、删除、URL 变更或配置重载，未写入/清理缓存，未启动 core、未发起 HTTP 请求、未使用 ADB，也未读取设备日志、配置、节点、URL、地址、路由、DNS、流量、凭据、Cookie、数据库或设备文件。

#### 本检查点远端备份状态（2026-09-10，FlClash subscription-userinfo 缓存失效、重命名与清理静态审计）

- focused commit `0d8e20d` 已创建但尚未 push；本检查点只涉及 capability parity matrix 与本 evidence 文件，其他工作树改动和临时产物未纳入。按要求不得自动 push，须先获得用户明确确认。
...
- focused commit `67533a9` 已创建但尚未 push；本检查点只涉及 capability parity matrix 与本 evidence 文件，其他工作树改动和临时产物未纳入。按要求不得自动 push，须先获得用户明确确认。

### FlClash DNS resolver 查询隐私与错误日志字段最小化静态审计（2026-09-10）

- 固定归档关键路径：`core/Clash.Meta/dns/client.go`、`core/Clash.Meta/dns/util.go`、`core/Clash.Meta/dns/resolver.go`、`core/Clash.Meta/dns/enhancer.go`；沿用既有 DNS 与加密 DNS 记录，本轮只补充查询元数据和诊断日志边界。
- 固定 resolver/client 路径的 debug/warn 日志可能携带 DNS server 地址、查询名、ECS 前缀、截断重试、缓存异常和原始 resolver error；未见字段白名单、host/query 哈希化、ECS 脱敏、调用者 scope 或默认关闭的诊断级别。
- 查询失败、截断重试、缓存未命中、上游拒绝和取消未形成稳定 `Unavailable`/`Cancelled`/`VersionMismatch` 分类，调用方无法从日志与 resolver 返回确认最小化映射，也不能证明某条日志对应哪一代配置或上游。
- 结论：XToolpro adapter 必须默认不记录原始 query、ECS、地址或上游错误，使用稳定脱敏错误码与可选采样计数，并分层返回 retry/cache/transport 原因；完成隐私日志、重试、取消和错误映射契约测试前保持 `Partial`，Proxy 台账保持 `Investigating`。
- 本轮仅基于固定归档记录进行静态审计，未发起 DNS/DoH/DoT/DoQ 查询，未读取设备日志、配置或网络内容，未启动 core、未使用 ADB，也未读取节点、请求、数据库、文件、凭据、Cookie、URL、地址、路由或流量内容。

#### 本检查点远端备份状态（2026-09-10，FlClash DNS resolver 查询隐私与错误日志字段最小化静态审计）
...
### FlClash 固定 core/service AAR bridge API 契约审计（2026-09-10）

- 范围：只读检查既有固定构建产物 `.tmp-flclash-aar-verify/classes.jar` 与 `.tmp-flclash-service-aar-verify/classes.jar`；分别为 `9,138` 与 `93,765` 字节，SHA-256 分别为 `D664DEFC1895F189EAE538FF7976DB6C03E4B508C9455EB493F1AD400AC1FD89` 与 `ABB60E8611AFBC9596535FBF0931F511F54490431415AD19C9EDF86DC15CF88C`。未修改 AAR、上游归档、SDK、缓存或构建产物。
- `jar tf` 与 `javap -public -s/-c -p` 确认 service AAR 的 `VpnService.LocalBinder.getService()` 暴露同进程 service 实例，`ManagedService.start()`/`stop()` 均返回 `void`。`VpnService.start()` 同步启动 service modules、读取 `ServiceConfig.vpnOptions`、建立 VPN descriptor 并调用 `Core.startTun()`；无返回值不携带 core health、TUN 收敛、generation 或版本 identity。
- `VpnService.stop()` 在 cleanup 后调用 `stopSelf()`；`ServiceModules.stop()` 逐个停止 module 时捕获 `Throwable`，只构造并丢弃 Kotlin `Result` 后继续。因此 binder 调用正常返回也不能证明所有 module 均已清理，现有 `ProxyCleanupStatus.COMPLETED` 不能直接由该返回值生成。
- core/service `classes.jar` 均未出现 `BuildConfig`、Version/Identity 或 terminal Result/Status/Health 类型。`VpnService.onStartCommand()` 发出的 `VPN_START_REQUESTED` 只表示请求到达，`VPN_REVOKED` 只表示系统撤销事件；二者都不是真实启动成功、停止完成或版本握手回执。
- 结论：真实 `FlClashRuntime` 候选必须把 AAR 正常返回解释为“同步调用未抛出”，另行绑定受签名的固定 artifact identity，并用不读取敏感内容的状态收敛探针确认 START/STOP；任何 module cleanup 异常都必须归入稳定、脱敏结果。完成真实 bridge 的 success、unavailable、cancel、crash、version mismatch 测试前，VPN 行保持 `Partial`，Proxy 台账保持 `Investigating`。
- 本轮未使用 ADB，未读取设备日志、配置、通知正文或 extras、节点、请求、数据库、文件、凭据、Cookie、订阅 URL、地址、路由、DNS 或流量内容；gate 数量保持 `Verified=3`、`Partial=161`、`Pending=29`、`Unavailable=1`、`Blocked=0`。

#### 本检查点远端备份状态（2026-09-10，FlClash 固定 core/service AAR bridge API 契约审计）
...
### FlClash `ProxyEngineIdentity` ABI/artifact manifest 版本握手契约（2026-09-10）
...
- 文档一致性复核：capability parity matrix 的 `engine-proxy` 行已同步为当前 10 个单测，并明确记录 contract/ABI/artifact manifest mismatch 与 identity/runtime `LinkageError` 覆盖；该同步不改变能力状态或 gate 计数。
...
### FlClash engine request task ID 非空边界契约（2026-09-10）

- TDD RED：先新增 `blankTaskIdIsRejectedAtRequestBoundary`，在 `ProxyEngineRequest` 尚未校验时执行专门测试，1 个测试按预期失败（`Expected exception`），证明空白 task ID 当前会穿过请求边界。
- GREEN：`ProxyEngineRequest` 构造边界新增 `require(taskId.isNotBlank())`；随后执行 `$env:GRADLE_USER_HOME='D:\\xtoolpro\\.gradle-user-home'; .\\gradlew.bat :engine-proxy:testDebugUnitTest --tests com.steveliuyan.xtoolpro.engine.proxy.FlClashProxyEngineAdapterTest --offline --rerun-tasks`，11 tests、0 failures、0 errors、0 skipped，Gradle `BUILD SUCCESSFUL in 29s`。
- 范围与局限：该检查只拒绝空字符串/全空白 ID，避免结果无法关联；不实现跨进程唯一性、持久化、重复提交幂等或取消状态机，这些仍需后续 task platform/engine integration 负责。未引入 FlClash SDK/native/AAR，也未进行设备 mutation。
- gate 数量保持 `Verified=3`、`Partial=161`、`Pending=29`、`Unavailable=1`、`Blocked=0`；Proxy 台账保持 `Investigating`，相关 engine 行保持 `Partial`。

#### 本检查点远端备份状态（2026-09-10，FlClash engine request task ID 非空边界契约）
...
### FlClash adapter identity/runtime failure 与 STOP 转发回归契约（2026-09-10）

- 本轮新增 `identityRuntimeFailureDoesNotEscapeTheEngineBoundary` 与 `stopRequestIsForwardedToRuntime` 两个静态回归测试；前者验证 identity handshake 的普通运行时异常只返回脱敏 `EngineCrashed(RUNTIME_FAILURE)`，后者验证 `STOP` 不会被错误降级为默认 `START`。
- GREEN：执行 `$env:GRADLE_USER_HOME='D:\\xtoolpro\\.gradle-user-home'; .\\gradlew.bat :engine-proxy:testDebugUnitTest --tests com.steveliuyan.xtoolpro.engine.proxy.FlClashProxyEngineAdapterTest --offline --rerun-tasks`，22 tests、0 failures、0 errors、0 skipped，Gradle `BUILD SUCCESSFUL in 29s`。
- 范围与局限：该轮只验证 adapter 的 identity/runtime 异常分类和操作参数转发，未加载真实 FlClash runtime，不证明 native crash、VPN/TUN、core 健康、配置或流量状态；未引入 SDK/native/AAR，未进行设备 mutation。
- gate 数量保持 `Verified=3`、`Partial=161`、`Pending=29`、`Unavailable=1`、`Blocked=0`；Proxy 台账保持 `Investigating`，相关 engine 行保持 `Partial`。

#### 本检查点远端备份状态（2026-09-10，FlClash adapter identity/runtime failure 与 STOP 转发回归契约）

- 本检查点将以只包含 `engine-proxy` 单元测试、capability parity matrix 与本 evidence 文件的 focused commit 提交，尚未 push；其他工作树改动和临时产物不纳入。按要求不得自动 push，须先获得用户明确确认。

### FlClash VersionMismatch receipt identity 差异边界契约（2026-09-10）

- TDD RED：新增 `equalIdentitiesAreRejectedForVersionMismatchReceipt`；在 `ProxyEngineResult.VersionMismatch` 尚未校验时，23 个测试中 1 个按预期失败（`Expected exception`），证明相同 identity 可被错误包装为版本不匹配终态。
- GREEN：`VersionMismatch` 构造边界现在要求 `expected` 与 `actual` identity 不相等；随后执行 `$env:GRADLE_USER_HOME='D:\\xtoolpro\\.gradle-user-home'; .\\gradlew.bat :engine-proxy:testDebugUnitTest --tests com.steveliuyan.xtoolpro.engine.proxy.FlClashProxyEngineAdapterTest --offline --rerun-tasks`，23 tests、0 failures、0 errors、0 skipped，Gradle `BUILD SUCCESSFUL in 29s`。
- 范围与局限：该检查只防止自相矛盾的版本不匹配回执，不证明 identity 的 commit、ABI、artifact manifest 来自签名 provenance，也不执行真实 bridge/native handshake。未引入 FlClash SDK/native/AAR，未进行设备 mutation。
- gate 数量保持 `Verified=3`、`Partial=161`、`Pending=29`、`Unavailable=1`、`Blocked=0`；Proxy 台账保持 `Investigating`，相关 engine 行保持 `Partial`。

#### 本检查点远端备份状态（2026-09-10，FlClash VersionMismatch receipt identity 差异边界契约）
