...
- 本轮只读审计固定 FlClash 提交；未执行 ADB 触控、未改变 VPN、配置或设备状态，未读取请求/连接、日志、网络标识、通知、配置、节点、订阅 URL、凭据、Cookie、设备数据库或导出文件。
- XToolpro 的 future `engine-proxy` 必须将“停止请求已提交”与“实际停止已核验”分为不同状态：await 受限的停止结果，并在实际停止完成或失败/超时时冻结事件入口、清理诊断队列、给出稳定脱敏错误。需以无敏感字段的设备断言覆盖成功、失败、超时、取消、engine crash 与 version mismatch；完成前矩阵保持 `Partial`，Proxy 台账保持 `Investigating`，不进入正式 engine 集成。
